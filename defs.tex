...
% If changing \parindent, one may want to reconsider the skips used in
% \problem so that the problem description indent matches the parindent.
\parindent=0pt
...
\def\imp{\;\Longrightarrow\;} % \imp to match \iff.

\newskip\hugeskipamount \hugeskipamount=24pt plus8pt minus8pt
\def\hugeskip{\vskip\hugeskipamount}
\def\xvskip#1{\par\ifdim\lastskip<#1\removelastskip\vskip#1\fi}
...
\long\def\problem#1\par#2\endproblem{{%
	\par
	\skip255=\lastskip
	\ifdim\skip255<\bigskipamount\removelastskip\fi
	\vskip 0pt plus100pt \penalty0 \vskip 0pt plus-100pt
	\ifdim\skip255<\bigskipamount\bigskip\fi
	\nointerlineskip\hbox{\twelvebf#1}%
	\nobreak\medskip
	\setbox0=\vbox{%
		\advance \hsize by -32pt
...
% Loop over tokens in #1, inserting #2 after each one.
\def\tokloop#1#2{{%
...
\def\spread#1#2{\tokloop{#1}{\hskip #2\relax}\unskip}
...
\newdimen\pc@rulewd
\newskip\pc@kwskip        \pc@kwskip=0.5em
...
\def\beginpcode{%
	...
	\pc@penalty=0
	\pc@lineno=0
	\setbox0=\hbox{$\ssty\pc@linenofmt$}% Assume digits are all same width.
	\parindent=\wd0
	\advance \parindent by 0.75em% Distance from line numbers.
	\setbox\pc@comment=\box\voidb@x
	\pc@rulewd=\hsize
	\advance \pc@rulewd by -\leftskip
	\advance \pc@rulewd by -\rightskip
	\advance \pc@rulewd by 4pt
	\bigbreak\pc@dorule\kern2pt}
\def\endpcode{%
	\kern2pt \pc@dorule\bigbreak
...
\def\pc@dorule{%
	...
	\hbox to \hsize{%
		\hss\hskip\leftskip
		\vrule height0.4pt width\pc@rulewd
		\hskip\rightskip\hss}%
	\nointerlineskip}
\def\pc@dokwskip{\hskip\pc@kwskip}
...
\def\Func#1(#2){%
	\Kw{function}\pc@dokwskip\Name{#1}(#2);
	\pc@addpenalty\pc@clubpenalty
	\pc@enter}
\def\If#1\Then{%
	\Kw{if}\pc@dokwskip #1\unskip\pc@dokwskip\Kw{then};
	\pc@addpenalty\pc@clubpenalty
	\pc@enter}
\def\Elseif#1\Then{%
	\pc@exit
	\pc@addpenalty\pc@widowpenalty
	\Kw{elseif}\pc@dokwskip #1\unskip\pc@dokwskip\Kw{then};
	\pc@addpenalty\pc@clubpenalty
	\pc@enter}
\def\Else{%
	\pc@exit
...
\def\For$#1$#2\From$#3$#4\To$#5$#6\Do{%
	\Kw{for}\pc@dokwskip$#1 := #3 \to #5$\unskip\pc@dokwskip\Kw{do};
	\pc@addpenalty\pc@clubpenalty
	\pc@enter}
\def\Foreach#1\In#2\Do{%
	\Kw{foreach}\pc@dokwskip #1\unskip\pc@dokwskip
	\Kw{in}\pc@dokwskip #2\unskip\pc@dokwskip\Kw{do};
	\pc@addpenalty\pc@clubpenalty
	\pc@enter}
\def\While#1\Do{%
	\Kw{while}\pc@dokwskip #1\unskip\pc@dokwskip\Kw{do};
	\pc@addpenalty\pc@clubpenalty
	\pc@enter}
\def\End{%
	\pc@exit
	\pc@addpenalty\pc@widowpenalty
	\Kw{end};}
\def\Return{\Kw{return}\pc@dokwskip}
\def\Kw#1{\leavevmode{\bf #1}}
\def\Id#1{\leavevmode{\it #1}}
\def\Name#1{\leavevmode{\sc #1}}
\def\Call#1(#2){\leavevmode\hbox{\Name{#1}(#2)}}
\def\And{\unskip\pc@dokwskip\Kw{and}\pc@dokwskip}
\def\Or{\unskip\pc@dokwskip\Kw{or}\pc@dokwskip}
\def\True{{\rm true}}
\def\False{{\rm false}}

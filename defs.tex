...
% Disable parskip until the next paragraph.
% Changes are local to group.
\newskip\nps@oldparskip
\def\noparskip{%
	\ifx\nps@oldpar\@undefined
		\let\nps@oldpar=\par
...
\def\problem#1\par{\@problem\prob@topskip{\twelvebf#1}}
\def\subproblem#1\par{\@problem\prob@topskip{\tenbf#1}}
\def\subproblemx#1\par{\@problem{0pt}{\tenbf#1}}
\def\@problem#1#2{%
	\par\begingroup
	...
	\setbox0=\vbox\bgroup
		...
		\ninept\sl\noindent\strut}
\def\endproblem{%
	\strut\egroup% End the vbox started in \@problem.

...
% TODO: genericize \verb macro by making begining and end of line tokens
%   arbitrary (see CSC349A A1)
% TODO: digit alignment macros from csc349a a1?
% TODO: new fonts from ENGR240
% TODO: check if non-scaled fonts are pixelated... some MF fonts are
% TODO: insertion macros that use \@vskip macros
% TODO: play with idea of locally redefining \end for various
%  "environment-style" macros (pcode, prob, etc.)
% TODO: lorem should not start new paragraph?
% TODO: macro for changing abovedisplayshortskip/abovedisaplayskip at the same
%   time
% TODO: put \relax in each of \rm,\bf,etc., in case they are first in halign entry.
% TODO: \page macro?
% TODO: general section macros with parametric fonts (plaintex beginsection
%   sucks): see engr240 memo
% TODO: more general font switch macros, e.g., do \fswap11 to switch to 11pt or
%   something (\eleventpt is akward name)
% TODO: more general figure macros (with caption support)?
% TODO: play arround with making slides with \special{papersize}/pdftex
% TODO: use a different char from plain tex for private macros? E.g., * or !
% TODO: play with \def\sel#1/#2/{} macro idea (select #1-th token out of #2)
% TODO: extension fonts at different sizes
% TODO: lipsum: option for long, medium, short paragraphs?
% TODO: \defsrc and \cite macros
% TODO: terser reinput protection using \csname autodef to \relax feature
...
% TODO: comment that macros are allowed to clobber \@, \@@, etc.
...
\newcount\@fori
\newcount\@form
\def\@foru{\ifnum\@fori<\@form \@forbody\advance\@fori 1 \ea\@foru \fi}
\def\@ford{\ifnum\@fori>\@form \@forbody\advance\@fori-1 \ea\@ford \fi}
\def\for#1=#2..#3\do#4{{%
	\let#1=\@fori \@fori=#2\relax \futurelet\@foreq\@form#3\relax
	\def\@forbody{#4}%
	\ifnum\@fori<\@form\@foru\else\@ford\fi
	\if\@foreq=\@forbody\fi}}

\def\@foragain{\ifx\@next\relax \else \ea\@iter \fi}
\def\foreach#1=#2\do#3{{%
	\def\@iter##1,{\def#1{##1}#3\futurelet\@next\@foragain}%
	\@iter#2,\relax}}
...
\def\blist{\let\ifmt=\@blist \icnt=0  \advance\ilvl1}
\def\nlist{\let\ifmt=\@nlist \icnt=0  \advance\ilvl1}
\def\ulist{\let\ifmt=\@clist \icnt=64 \advance\ilvl1}
\def\llist{\let\ifmt=\@clist \icnt=96 \advance\ilvl1}
\def\item{\par \advance\icnt1 \global\hangafter=0
	\global\hangindent=\iind \global\multiply\hangindent\ilvl
	\global\advance\hangindent\ioff \noindent\llap{\ifmt}\ignorespaces}
